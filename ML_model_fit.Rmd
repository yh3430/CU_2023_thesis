...
# Step 3, fit the data with multinominal logistic regression, random forest, SVM, classfication Tree, Elastic Net, ridge, lasso regression, and classification tree
...
```{r}
# the test dataset
df_state_test <- df_fit_state %>%
  select(-sitename) %>%
  select(-year)

# model random forest
rf_outcome_test <- predict(model_rf, df_state_test, type = "raw")
rf_evaluation_test <- confusionMatrix(rf_outcome_test, df_state_test$q85, positive = "1")
rf_evaluation_test

postResample(rf_outcome_test, df_state_test$q85)
```
