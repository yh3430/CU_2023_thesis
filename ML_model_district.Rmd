...
```{r}
df_district_train <- df_fit_district %>%
  select(-year)

summary(df_district_train)
```
...
multinom_outcome <- predict(model_multinom, df_district_train, type = "raw")

# Create confusion matrix
multinom_evaluation <- confusionMatrix(multinom_outcome, df_district_train$q85)
multinom_evaluation
```
...
#mtry_vals_rf <- c(ncol(df_district_train)-1, 
#                 sqrt(ncol(df_district_train)-1), 
#                  0.5*ncol(df_district_train)-1)
#mtry_grid_rf<- expand.grid(.mtry = round(mtry_vals_rf))

#model_rf <- train(q85 ~., data = df_district_train, 
...
## ntree = 200 give the best results
```{r}
# ntree = 200
set.seed(123)
# Setting 5-fold cross-validation for fast demonstration.
...
rf_outcome <- predict(model_rf, df_district_train, type = "raw")
confusionMatrix(rf_outcome, df_district_train$q85)
```
...
svm_outcome <- predict(model_svc, df_district_train, type = "raw")
confusionMatrix(svm_outcome, df_district_train$q85)
```
...
lasso_outcome <- predict(model_lasso, df_district_train, type = "raw")
confusionMatrix(lasso_outcome, df_district_train$q85)
```
...
ridge_outcome <- predict(model_ridge, df_district_train, type = "raw")
confusionMatrix(ridge_outcome, df_district_train$q85)
```
...
elastic_net_outcome <- predict(model_elastic_net, df_district_train, type = "raw")
confusionMatrix(elastic_net_outcome, df_district_train$q85)
```
...
tree_outcome <- predict(model_tree, df_district_train, type = "raw")
confusionMatrix(tree_outcome, df_district_train$q85)
```
...
multinom_outcome <- predict(model_multinom, df_district_train, type = "raw")
multinom_evaluation <- confusionMatrix(multinom_outcome, df_district_train$q85, positive = "1")
multinom_evaluation

# model random forest
rf_outcome <- predict(model_rf, df_district_train, type = "raw")
rf_evaluation <- confusionMatrix(rf_outcome, df_district_train$q85, positive = "1")
rf_evaluation

# model SVM
svm_outcome <- predict(model_svc, df_district_train, type = "raw")
svm_evaluation <- confusionMatrix(svm_outcome, df_district_train$q85, positive = "1")
svm_evaluation

# model lasso
lasso_outcome <- predict(model_lasso, df_district_train, type = "raw")
lasso_evaluation <- confusionMatrix(lasso_outcome, df_district_train$q85, positive = "1")
lasso_evaluation

# model ridge
ridge_outcome <- predict(model_ridge, df_district_train, type = "raw")
ridge_evaluation <- confusionMatrix(ridge_outcome, df_district_train$q85, positive = "1")
ridge_evaluation

# model elastic net
elastic_net_outcome <- predict(model_elastic_net, df_district_train, type = "raw")
elastic_net_evaluation <- confusionMatrix(elastic_net_outcome, df_district_train$q85, positive = "1")
elastic_net_evaluation

# model classification tree
tree_outcome <- predict(model_tree, df_district_train, type = "raw")
tree_evaluation <- confusionMatrix(tree_outcome, df_district_train$q85, positive = "1")
tree_evaluation
```
...
postResample(multinom_outcome, df_district_train$q85)
postResample(rf_outcome, df_district_train$q85)
postResample(svm_outcome, df_district_train$q85)
postResample(lasso_outcome, df_district_train$q85)
postResample(ridge_outcome, df_district_train$q85)
postResample(elastic_net_outcome, df_district_train$q85)
postResample(tree_outcome, df_district_train$q85)
```
...
# model random forest
rf_outcome_test <- predict(model_rf, df_district_train, type = "raw")
rf_evaluation_test <- confusionMatrix(rf_outcome_test, df_district_train$q85, positive = "1")
rf_evaluation_test

postResample(rf_outcome_test, df_district_train$q85)
```

# Step 6, run the best random forest model for every school district
```{r}
# Get unique values of sitname
unique_sitenames <- unique(df_district_train$sitename)
...
for (sitename in unique_sitenames) {
  ...
  df_train_sitename <- df_district_train[df_district_train$sitename == sitename, ]
...
for (sitename in unique_sitenames) {
  ...
  # Make predictions on the test dataset
  lasso_outcome_test <- predict(model_lasso, df_train_sitename, type = "raw")
  
  # Evaluate the model using the confusionMatrix function
  lasso_evaluation_test <- confusionMatrix(lasso_outcome_test,  df_train_sitename$q85, positive = "1")
  
  # Calculate performance metrics using the postResample function
  performance_metrics <- postResample(lasso_outcome_test, df_train_sitename$q85)
  
  # Get variable importance
  variable_importance <- varImp(model_lasso)
  
  # Store the results for the current sitname in the results list
  results[[sitename]] <- list(
    "ConfusionMatrix" = lasso_evaluation_test,
    "PerformanceMetrics" = performance_metrics,
    "VariableImportance" = variable_importance
  )
}
...
lasso_outcome <- predict(model_lasso, df_st, type = "raw")
confusionMatrix(lasso_outcome, df_st$q85)
```
